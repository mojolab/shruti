...
CORS(app)
me = MongoEngine(app)
app.logger = xpal.seebotxpal.logger
...
class MemberResource(Resource):
    def get(self, tgid=None, mobile_num=None, docid=None, member_id=None, command=None):
        if command is not None:
            app.logger.info(
                "MemberResource: Received Command {}".format(command))
            if command == "export":
                try:
                    resp = xpal.export_members()
                    status = "success"
            ...
            resp = [xpal.documents.documents.Member.objects.with_id(docid)]
        elif tgid is not None:
            resp = list(xpal.documents.Member.objects(tgid=tgid))
        elif mobile_num is not None:
            resp = list(xpal.documents.Member.objects(mobile_num=mobile_num))
        elif member_id is not None:
            resp = list(xpal.documents.Member.objects(member_id=member_id))
        else:
            resp = list(xpal.documents.Member.objects.all())
    ...
    def post(self, command=None):
        ...
        if command is None:
            try:
                if xpal.validate_member_dict(respdict)['status'] is True:
                    resp = xpal.create_member(respdict)
                    if type(resp) != list:
                        status = "error"
                    else:
                        status = "success"
                    ...
                    status = "error"
                    resp = xpal.validate_member_dict(respdict)['message']
            except Exception as e:
                app.logger.error("{} {}".format(type(e), str(e)))
                resp = "{} {}".format(type(e), str(e))
                status = "error"
            ...
            try:
                ...
                resp = xpal.import_members(respdict)
                if type(resp) != list:
                    status = "error"
                else:
                    status = "success"
            ...
            try:
                if type(respdict) != list:
                    status = "error"
                    resp = "Bulk Delete Expects a list of member ids"
                else:
                    deletedmembers = []
                    for member_id in respdict:
                        retval = xpal.delete_member(member_id)
                        if retval == []:
                            deletedmembers.append(member_id)
                    resp = "Deleted members {} of {}".format(deletedmembers, respdict)
                    if deletedmembers == respdict:
                        status = "success"
                    else:
                        status = "error"
            except Exception as e:
    ...
    def put(self, member_id):
        ...
        respdict = request.get_json()
        try:
            if xpal.validate_member_dict(respdict, new=False)['status'] is True:
                resp = xpal.update_member(member_id, respdict)
                if type(resp) != list:
                    status = "error"
                else:
                    status = "success"
                ...
                status = "error"
                resp = xpal.validate_member_dict(
    ...
    def delete(self, member_id=None):
        if member_id is None:
            ...
            app.logger.info(
                "MemberResource: Trying to delete member {}".format(member_id))
            try:
                resp = xpal.delete_member(member_id)
                if type(resp) == list:
                    status = "success"
                else:
                    status = "error"
...
api.add_resource(MemberResource, "/member", endpoint="member")
api.add_resource(MemberResource, "/member/by_tgid/<int:tgid>", endpoint="tgid")
api.add_resource(
    MemberResource, "/member/by_mobile/<string:mobile_num>", endpoint="mobile")
api.add_resource(MemberResource, "/member/by_id/<string:docid>",
                 endpoint="member_docid")
api.add_resource(
    MemberResource, "/member/by_member_id/<string:member_id>", endpoint="memberid")
api.add_resource(MemberResource, "/member/<string:command>",
                 endpoint="member_command")
    ...
    def post(self):
        ...
        try:
            if xpal.validate_locupdate_dict(respdict)['status'] is True:
                member = xpal.documents.Member.objects(
                    member_id=respdict["member_id"])[0]
                ...
                locupdate = xpal.new_locationupdate(
                    member, timestamp, vehicle=vehicle)
...
class BookingResource(Resource):
    def get(self, docid=None, booking_id=None, command=None):
        if command is not None:
            ...
            resp = list(xpal.documents.Booking.objects.order_by("-created_timestamp").paginate(page=1, per_page=100).items)
        if type(resp) == list and resp != []:
            status = "success"
        else:
            status = "error"
    ...
    def post(self, command=None):
        ...
        if command is None:
            ...
            try:
                ...
                queryset = queryset.order_by("-created_timestamp").paginate(page=1, per_page=100).items
                assignmentlist = []
...
class DutySlipResource(Resource):
    def get(self, docid=None, assignment_id=None, member_id=None):
        if docid is not None:
            ...
            resp = xpal.documents.DutySlip.objects(member=member_id)
        else:
            app.logger.info("Returning DS sorted by created_time")
            resp = xpal.documents.DutySlip.objects.order_by("-created_time").paginate(page=1, per_page=100).items
        if resp == []:
            status = "error"
        else:
            status = "success"
...
api.add_resource(DutySlipResource, "/dutyslip/by_member_id/<string:member_id>",
                 endpoint="dutyslip_memberid")
